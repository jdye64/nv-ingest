...
"""
Two-stage batch ingestion pipeline.

Stage 1 -- Extract: runs PDF split, extraction, page-element detection, and
OCR with exclusive access to all GPUs (no embedding actor competing for VRAM).
The result is materialized in memory.

Stage 2 -- Embed + VDB Upload: takes the materialized extraction dataset,
runs text embedding, and streams rows into LanceDB.

Run with: uv run python -m nemo_retriever.examples.batch_pipeline_staged <input-dir>
"""
...
def main(
    ...
    try:
        ...
        # ---------------------------------------------------------------
        # Shared param objects used in both stages
        # ---------------------------------------------------------------
        ...
        # ---------------------------------------------------------------
        # Non-PDF pipelines (txt / html) -- single-stage, no GPU contention
        # ---------------------------------------------------------------
        if input_type == "txt":
            ingestor = (
            ...
            logger.info("Running single-stage txt pipeline...")
            ...
            logger.info("Running single-stage html pipeline...")
            ingest_start = time.perf_counter()
            ingest_results = (
                ingestor.ingest(
                    params=IngestExecuteParams(
            ...
            # -----------------------------------------------------------
            # PDF / doc pipelines -- two-stage execution
            # -----------------------------------------------------------
            # Stage 1: Extract (page-elements, OCR, etc.) -- gets all GPUs
            ingestor.files(file_patterns).extract(extract_params)

            logger.info("Stage 1/2: Running extraction (page-elements + OCR) with exclusive GPU access...")
            ...
            extract_rows = extract_ds.count()
            logger.info(
                "Stage 1 complete: %d rows extracted in %.2f s (%.2f pages/s)",
                extract_rows,
                extract_elapsed,
                extract_rows / max(extract_elapsed, 1e-6),
            )

            # Stage 2: Embed + VDB Upload -- gets all GPUs (extraction actors gone)
            logger.info("Stage 2/2: Running embedding + VDB upload with exclusive GPU access...")
            ingest_start = time.perf_counter()
            ingest_results = (
                ingestor.embed(embed_params, input_dataset=extract_ds)
                .vdb_upload(vdb_params)
                .ingest(
                    params=IngestExecuteParams(
                        runtime_metrics_dir=str(runtime_metrics_dir) if runtime_metrics_dir is not None else None,
