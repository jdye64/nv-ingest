...
def build_embed_option_kwargs(
    ...
    if embed_enforce_eager is not None:
        runtime_kwargs["enforce_eager"] = bool(embed_enforce_eager)
    if embed_max_length is not None:
        runtime_kwargs["max_length"] = int(embed_max_length)
    if runtime_kwargs:
        from nemo_retriever.common.params.models import ModelRuntimeParams

        embed_kwargs["runtime"] = ModelRuntimeParams(**runtime_kwargs)
