    ...
    *,
    ...
    if existing is not None:
        if job_id is not None and existing.job_id != job_id:
            # Treat dup-across-jobs as accepted: count toward this job's tally.
            repo.increment_job_pages_submitted(job_id)
    ...
    if lost_race:
        # Some other request beat us to the SHA-unique row; treat the
        # whole submit as a dedup hit so we don't double-submit the page.
...
async def ingest_batch(
    ...
    job_id = meta.job_id or uuid.uuid4().hex
    if not meta.job_id:
        # Caller didn't pre-create a job; create one to group the uploads.
