    ...
    async def _drain(self) -> None:
        ...
        try:
            async for item in agen:
                # Bounded queue: poll-with-timeout + stop-event check so the
                # producer thread can shut down promptly if the consumer
                # broke out of its for-loop early.
                while True:
...
class ServiceIngestor(ingestor):
    ...
    def pdf_split_config(self, pages_per_chunk: int = 32) -> "ServiceIngestor":
        # Server splits per-page already; this knob has no effect today.
