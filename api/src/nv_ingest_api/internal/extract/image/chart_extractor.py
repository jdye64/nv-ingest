...
def _validate_chart_inference_results(
    yolox_results: Any, rtx_translate_results: Any, valid_arrays: List[Any], valid_images: List[str]
) -> Tuple[List[Any], List[Any]]:
    """
    ...
    """
    if not (isinstance(yolox_results, list) and isinstance(rtx_translate_results, list)):
        raise ValueError("Expected list results from both yolox_client and rtx_translate_client infer calls.")
...
def _merge_chart_results(
    ...
    rtx_translate_results: List[Any],
    ...
    for idx, (yolox_res, rtx_translate_res) in enumerate(zip(yolox_results, rtx_translate_results)):
        # Unpack rtx_translate result into bounding boxes and text predictions.
        bounding_boxes, text_predictions = rtx_translate_res
...
def _update_chart_metadata(
    ...
    yolox_results, rtx_translate_results = _run_chart_inference(
    ...
    yolox_results, rtx_translate_results = _validate_chart_inference_results(
        yolox_results, rtx_translate_results, valid_arrays, valid_images
    )

    # Merge the inference results into the results list.
    return _merge_chart_results(base64_images, valid_indices, yolox_results, rtx_translate_results, results)
...
def extract_chart_data_from_image_internal(
    ...
    try:
        ...
        try:
            if rtx_translate_client is not None:
