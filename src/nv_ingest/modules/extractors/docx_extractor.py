...
"""
Module for extracting content from docx
"""
...
MODULE_NAME = "docx_content_extractor"
MODULE_NAMESPACE = "nv-ingest"
DocxExtractorLoaderFactory = ModuleLoaderFactory(MODULE_NAME, MODULE_NAMESPACE, PDFExtractorSchema)
...
def _process_docx_bytes(df, task_props):
    ...
    Parameters:
    - df: cuDF DataFrame with columns 'source_id' and 'content' (base64 encoded documents).

    Returns:
    - A cuDF DataFrame with the docx content replaced by the extracted text.
    """
...
def _worker_target(recv_queue, send_queue, **kwargs):
    ...
    while not cancellation_token.value:
        ...
        df, task_props = work
        result = _process_docx_bytes(df, task_props)

        send_queue.put(result)


@register_module(MODULE_NAME, MODULE_NAMESPACE)
def _docx_text_extractor(builder: mrc.Builder):
    ...
    @filter_by_task([("extract", {"document_type": "docx"})])
    ...
    input_node = builder.make_node("docx_content_extractor", ops.map(no_op))
