        ...
        "inputs": [
            {
                "name": "candidates",
                "datatype": "BYTES",
                "shape": [len(inputs), n_candidates],
                "data": _sanitize_inputs(inputs),
            }
        ],
        "outputs": [{"name": "caption", "datatype": "BYTES", "shape": [len(inputs), 1]}],
    }

    json_input = json.dumps(input_data, indent=2)
    caption = [""] * len(inputs)
    try:
        response = requests.post(triton_url, headers=headers, data=json_input)
        if response.status_code in [400, 500]:
            error_info = response.json().get("error", "No additional error information")
            logging.error(f"Server returned status code {response.status_code}: {error_info}")
            return caption

        response.raise_for_status()
        response_json = response.json()
        caption = response_json["outputs"][0]["data"]
