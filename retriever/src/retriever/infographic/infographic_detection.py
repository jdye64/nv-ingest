...
def _decode_b64_image_to_chw_tensor(image_b64: str) -> Tuple["torch.Tensor", Tuple[int, int]]:
    return _shared_decode_b64_image_to_chw_tensor(
        image_b64,
        import_error_message="infographic detection requires torch, pillow, and numpy.",
    )
    ...
    def __call__(self, batch_df: Any, **override_kwargs: Any) -> Any:
        ...
        def _df_error(df: pd.DataFrame, exc: BaseException) -> Any:
            ...
            return apply_dataframe_defaults(
                df,
                column_defaults={
                    "infographic_elements_v1": lambda: {"regions": [], "timing": None, "error": payload.get("error")},
                    "infographic_elements_v1_num_detections": 0,
                    "infographic_elements_v1_counts_by_label": dict,
                },
            )

        def _other_error(exc: BaseException) -> Any:
            return [{"infographic_elements_v1": _error_payload(stage="actor_call", exc=exc)}]
