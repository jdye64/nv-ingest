...
| nemotron-parse | `https://integrate.api.nvidia.com/v1/chat/completions` with model ID `nvidia/nemotron-parse` | Optional `method="nemotron_parse"`. Hosted Build and self-hosted `nemotron-parse-v2.0` use different request contracts; the library selects the matching contract automatically. Refer to [Nemotron Parse: hosted Build endpoint vs self-hosted NIM](#nemotron-parse-hosted-vs-self-hosted) |
...
!!! note "Nemotron Parse: hosted Build endpoint vs self-hosted NIM"
    ...
    - **Self-hosted chat endpoints** default to model ID `nvidia/nemotron-parse-v2.0` and use the tagged text-prompt contract.
