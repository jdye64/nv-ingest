...
def test_redis_task_source_schema_defaults():
    ...
    assert (
        schema.task_queue == "morpheus_task_queue"
    ), "Default value for task_queue should be 'morpheus_task_queue'."
    assert (
        schema.progress_engines == 6
    ), "Default value for progress_engines should be 6."
...
def test_redis_task_source_schema_custom_values():
    ...
    """
    custom_redis_client = RedisClientSchema(
        host="custom_host", port=12345, use_ssl=True
    )
    schema = RedisTaskSourceSchema(
        redis_client=custom_redis_client, task_queue="custom_queue", progress_engines=10
    )

    assert (
        schema.redis_client.host == "custom_host"
    ), "Custom host value for redis_client should be respected."
    assert (
        schema.redis_client.port == 12345
    ), "Custom port value for redis_client should be respected."
    assert (
        schema.redis_client.use_ssl is True
    ), "Custom use_ssl value for redis_client should be True."
    assert (
        schema.task_queue == "custom_queue"
    ), "Custom value for task_queue should be respected."
    assert (
        schema.progress_engines == 10
    ), "Custom value for progress_engines should be respected."
