...
from nv_ingest_client.client.exception import JobSubmissionException
...
class NvIngestClient:
    """
    A client class for interacting with the nv-ingest service.
    """

    def __init__(
            self,
            hostname: Optional[str] = "localhost",
            port: Optional[int] = 7670,
        ...
        port : int, optional
            The port of the REST server. Defaults to 7670.
        ...
        self._job_states = {}
        self._hostname = hostname or "localhost"
        self._port = port or 7670
    ...
    def submit(self, timeout: int = 30):
        print("Submit the job")
        url = f"http://{self._hostname}:{self._port}{self._submit_endpoint}"
        logger.debug(f"Invoking fetch_message http endpoint @ '{url}'")
        
        try:
            result = requests.post(url, json={"payload": message}, headers={"Content-Type": "application/json"}, timeout=timeout)
            return result
        except Exception:
            raise JobSubmissionException("yep, we got a problem here!")
        
    def results(self, timeout: int = 30):
        print("Fetching job")
