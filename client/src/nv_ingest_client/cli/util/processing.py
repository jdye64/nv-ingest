...
def create_job_specs_for_batch(
    ...
    for file_name in files_batch:
        ...
        # # The RestClient has the job_id generated by the server so do not attempt that for it
        # if not isinstance(client._message_client, RestClient):
    ...
    with tqdm(total=total_files, desc="Processing files", unit="file") as pbar:
        ...
        while (processed < len(files)) or retry_job_ids:
            ...
            if (cur_job_count < batch_size) and (processed < len(files)):
                ...
                # # A RedisClient must provide the job_id where the RestClient the server
                # # generates the job_id and returns it in the response. Therefore we need
                # # to handle those discrepances until we only use RestClient.
                # if isinstance(client._message_client, RestClient):
                #     # While this would generally contain the `new_job_ids` that is not the case for the RestClient
                #     job_specs, _ = create_job_specs_for_batch(batch_files, tasks, client)
                #     breakpoint()
                #     print("here we go")
                #     # The current submission via the "client" will not work with HTTP. So we rely on a async method in the RestClient itself
                #     client._message_client.submit_message()
                # else:
