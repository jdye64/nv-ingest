...
def insert_license(f, this_year, first_line):
    ...
    try:
        ...
        return [
            ...
            "please manually insert an NVIDIA proprietary header or add the file to "
...
def check_copyright(
    f,
    update_current_year,
    verify_nv_proprietary_lic=False,
    ...
    line_num = 0
    cr_found = False
    verify_nv_proprietary_lic_found = not verify_nv_proprietary_lic
    year_matched = False
    with io.open(f, "r", encoding="utf-8") as file:
        lines = file.readlines()
    for line in lines:
        line_num += 1
        if not verify_nv_proprietary_lic_found:
            verify_nv_proprietary_lic_found = CHECK_PROPRIETARY_LIC in line
    ...
    if not verify_nv_proprietary_lic_found:
        if do_insert_license and len(lines):
            ...
            e = [
                f,
                0,
                "NVIDIA Proprietary copyright header missing, if this file needs to be "
                "exempted from this check add it to the 'ExemptFiles' list in "
    ...
    if year_matched and verify_nv_proprietary_lic_found:
        errs = []
...
def _main():
    ...
    """
    log_level = logging.getLevelName(os.environ.get("NV_INGEST_LOG_LEVEL", "INFO"))
    ...
    argparser.add_argument(
        ...
        required=False,
        help="If set, "
        "inserts the NVIDIA Proprietary license into a files "
        "without a license, implies --verify-nvidia-proprietary",
    )
    ...
    if len(errors) > 0:
        ...
        file_from_repo = os.sep.join(path_parts[path_parts.index("ci") :])  # noqa E203
        if n_fixable > 0:
            logging.info(
                (
