...
def _run_cmd(exe: str, *args: str):
    """Runs a command with args and returns its output, supports shell commands."""

    use_shell = "|" in args or ">" in args or "<" in args
    if use_shell:
        cmd_str = " ".join((exe,) + args)
        cmd_list = cmd_str  # For shell=True, the command must be a single string
    else:
        cmd_list = (exe,) + args
        cmd_str = " ".join(cmd_list)

    try:
        ret = subprocess.check_output(cmd_list, stderr=subprocess.PIPE, shell=use_shell)
        output = ret.decode("UTF-8").rstrip("\n")

        logging.debug("Running command: `%s`, Output: '%s'", cmd_str, output)
...
def get_first_common_ancestor(current_branch="HEAD"):
    try:
        # Run the git merge-base command to find the common ancestor between the current branch and origin/main
        merge_base_cmd = ["git", "merge-base", current_branch, "origin/main"]
        merge_base = subprocess.check_output(merge_base_cmd, text=True).strip()

        return merge_base
    except subprocess.CalledProcessError as e:
        logging.error(f"Error finding the first common ancestor: {e}")
        return None
...
def determine_merge_commit(current_branch="HEAD"):
    ...
    common_commit = get_first_common_ancestor(current_branch)
...
def modified_files(
    ...
    *,
    ...
    # TODO(Devin): Existing code assumes a remote naming structure we don't have yet -- we should move to cal-ver.
    return "origin/main"
...
def _main():
    ...
    if args.action == "get_merge_target":
        logging.info(determine_merge_commit())
